import base64
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad
import hashlib

def decode_hex_data(hex_string):
    """
    Decode hex data menggunakan ABI encoding rules
    """
    # Remove '0x' prefix if present
    if hex_string.startswith('0x'):
        hex_string = hex_string[2:]
    
    # Convert hex to bytes
    data = bytes.fromhex(hex_string)
    
    # Parse the data according to ABI encoding rules
    # First 4 bytes are function selector (skip for this case)
    # Next 32 bytes chunks contain the actual data
    
    def read_uint256(data, offset):
        return int.from_bytes(data[offset:offset+32], 'big')
    
    def read_address(data, offset):
        # Address is in the last 20 bytes of a 32-byte slot
        addr_bytes = data[offset+12:offset+32]
        return '0x' + addr_bytes.hex()
    
    def read_string(data, string_offset):
        # String length is stored in the first 32 bytes at the offset
        length = read_uint256(data, string_offset)
        # String data starts 32 bytes after the length
        string_data = data[string_offset+32:string_offset+32+length]
        return string_data.decode('utf-8')
    
    # Skip function selector (first 4 bytes)
    offset = 4
    
    # Read owner address (32 bytes)
    owner = read_address(data, offset)
    offset += 32
    
    # Skip string offset pointers (we'll read them later)
    # These are: name, instagram, twitter, telegram, title, description, image offsets
    name_offset_ptr = read_uint256(data, offset)
    offset += 32
    instagram_offset_ptr = read_uint256(data, offset)
    offset += 32
    twitter_offset_ptr = read_uint256(data, offset)
    offset += 32
    telegram_offset_ptr = read_uint256(data, offset)
    offset += 32
    title_offset_ptr = read_uint256(data, offset)
    offset += 32
    description_offset_ptr = read_uint256(data, offset)
    offset += 32
    
    # Read target (uint256)
    target = read_uint256(data, offset)
    offset += 32
    
    # Read deadline (uint256)
    deadline = read_uint256(data, offset)
    offset += 32
    
    # Read image offset pointer
    image_offset_ptr = read_uint256(data, offset)
    
    # Now read the actual strings using their offset pointers
    # Add 4 to account for function selector we skipped
    name = read_string(data, name_offset_ptr + 4)
    instagram = read_string(data, instagram_offset_ptr + 4)
    twitter = read_string(data, twitter_offset_ptr + 4)
    telegram = read_string(data, telegram_offset_ptr + 4)
    title = read_string(data, title_offset_ptr + 4)
    description = read_string(data, description_offset_ptr + 4)
    image = read_string(data, image_offset_ptr + 4)
    
    return {
        'owner': owner,
        'target': target,
        'deadline': deadline,
        'name': name,
        'instagram': instagram,
        'twitter': twitter,
        'telegram': telegram,
        'title': title,
        'description': description,
        'image': image
    }

def decrypt_aes_cryptojs(encrypted_data, password):
    """
    Mendekripsi data yang dienkripsi menggunakan CryptoJS AES format
    """
    try:
        # Decode base64
        encrypted_bytes = base64.b64decode(encrypted_data)
        
        # CryptoJS format: "Salted__" + 8 bytes salt + encrypted data
        if encrypted_bytes[:8] != b'Salted__':
            raise ValueError("Format enkripsi tidak valid")
        
        # Extract salt (8 bytes after "Salted__")
        salt = encrypted_bytes[8:16]
        
        # Extract encrypted data
        encrypted_content = encrypted_bytes[16:]
        
        # Derive key dan IV menggunakan MD5 (seperti CryptoJS default)
        def derive_key_iv(password, salt, key_len=32, iv_len=16):
            """
            Derive key dan IV seperti yang dilakukan CryptoJS
            Menggunakan EVP_BytesToKey algorithm dengan MD5
            """
            md5 = hashlib.md5
            password_bytes = password.encode('utf-8') if isinstance(password, str) else password
            
            m = []
            i = 0
            while len(b''.join(m)) < (key_len + iv_len):
                md5_hash = md5()
                data = password_bytes + salt
                if i > 0:
                    data = m[i - 1] + password_bytes + salt
                md5_hash.update(data)
                m.append(md5_hash.digest())
                i += 1
            
            ms = b''.join(m)
            key = ms[:key_len]
            iv = ms[key_len:key_len + iv_len]
            return key, iv
        
        # Derive key dan IV
        key, iv = derive_key_iv(password, salt)
        
        # Decrypt
        cipher = AES.new(key, AES.MODE_CBC, iv)
        decrypted = cipher.decrypt(encrypted_content)
        
        # Remove padding
        decrypted = unpad(decrypted, AES.block_size)
        
        return decrypted.decode('utf-8')
    
    except Exception as e:
        return f"Error: {str(e)}"

def main():
    # Data hex yang akan di-decode
    hex_data = ""
    
    print("STEP 1: DECODE HEX DATA")
    print("=" * 60)
    
    # Decode hex data
    result = decode_hex_data(hex_data)
    
    # Tampilkan hasil decode (dengan data terenkripsi)
    print(f"Owner: {result['owner']}")
    print(f"Title: {result['title']}")
    print(f"Description: {result['description']}")
    print(f"Target: {result['target']} wei ({result['target'] / 10**18} ETH)")
    print(f"Deadline: {result['deadline']} (timestamp)")
    print(f"Image: {result['image']}")
    print("\nDATA TERENKRIPSI:")
    print(f"Name: {result['name']}")
    print(f"Instagram: {result['instagram']}")
    print(f"Twitter: {result['twitter']}")
    print(f"Telegram: {result['telegram']}")
    
    print("\n" + "=" * 60)
    print("STEP 2: DECRYPT ENCRYPTED DATA")
    print("=" * 60)
    
    # Secret key untuk dekripsi
    secret_key = "MyS3cReTK3y"
    
    # Dekripsi data terenkripsi
    encrypted_fields = ['name', 'instagram', 'twitter', 'telegram']
    decrypted_data = {}
    
    for field in encrypted_fields:
        encrypted_value = result[field]
        decrypted_value = decrypt_aes_cryptojs(encrypted_value, secret_key)
        decrypted_data[field] = decrypted_value
        print(f"{field.capitalize()}: {decrypted_value}")
    
    print("\n" + "=" * 60)
    print("STEP 3: DATA LENGKAP SETELAH DEKRIPSI")
    print("=" * 60)
    
    # Tampilkan semua data lengkap
    print(f"Owner: {result['owner']}")
    print(f"Name: {decrypted_data['name']}")
    print(f"Title: {result['title']}")
    print(f"Description: {result['description']}")
    print(f"Target: {result['target']}")
    # print(f"Target: {result['target']} wei ({result['target'] / 10**18} ETH)")
    print(f"Deadline: {result['deadline']} (timestamp)")
    print(f"Image: {result['image']}")
    print(f"Instagram: {decrypted_data['instagram']}")
    print(f"Twitter: {decrypted_data['twitter']}")
    print(f"Telegram: {decrypted_data['telegram']}")

if __name__ == "__main__":
    main()