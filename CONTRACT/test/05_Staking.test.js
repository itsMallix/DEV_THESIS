...
describe("Staking Contract", function () {
  ...
  let addr1;
  let addr2;
  let timeUnit = 86400; // 1 day in seconds
  ...
  beforeEach(async function () {
    ...
    await stakingToken.connect(addr1).approve(stakingAddress, ethers.parseEther("10000"));
    await stakingToken.connect(addr2).approve(stakingAddress, ethers.parseEther("10000"));
  ...
  describe("Unstaking", function () {
    ...
    it("Should allow users to unstake tokens", async function () {
      ...
      expect(await staking.stakedBalances(addr1.address)).to.equal(ethers.parseEther("500"));
      expect(await staking.stakingTokenBalance()).to.equal(ethers.parseEther("500"));
    });

    it("Should not allow unstaking more than staked amount", async function () {
      const excessAmount = ethers.parseEther("1500");
      await expect(staking.connect(addr1).unstake(excessAmount)).to.be.revertedWith("Insufficient staked balance");
    });

    it("Should not allow unstaking if nothing is staked", async function () {
      await expect(staking.connect(addr2).unstake(ethers.parseEther("100"))).to.be.revertedWith("Insufficient staked balance");
    });
  });
  ...
  describe("Rewards", function () {
    beforeEach(async function () {
      ...
      await staking.connect(owner).depositRewardTokens(ethers.parseEther("10000"));
    });
    ...
    it("Should allow users to claim rewards", async function () {
      ...
      const differencePct = Math.abs(Number(actualDifference - calculatedReward)) / Number(calculatedReward);
      expect(differencePct).to.be.lessThan(0.001, "Balance difference should match calculated reward");
    });
    ...
    it("Should not allow claiming rewards with zero staked balance", async function () {
      await expect(staking.connect(addr2).claimRewards()).to.be.revertedWith("No staked tokens");
    });
    ...
    it("Should not allow claiming rewards if contract has insufficient reward tokens", async function () {
      ...
      await staking.connect(owner).withdrawRewardTokens(ethers.parseEther("10000"));
      
      // Advance time to accumulate rewards
      await time.increase(timeUnit);
      
      // Try to claim rewards
      await expect(staking.connect(addr1).claimRewards()).to.be.revertedWith("Not enough rewards");
    });
  });
  ...
  describe("Admin Functions", function () {
    ...
    it("Should not allow non-admin to deposit reward tokens", async function () {
      await expect(staking.connect(addr1).depositRewardTokens(ethers.parseEther("1000")))
        .to.be.revertedWith("Not authorized");
    });

    it("Should allow admin to withdraw reward tokens", async function () {
      // First deposit some tokens
      await staking.connect(owner).depositRewardTokens(ethers.parseEther("5000"));
      ...
      expect(await staking.rewardTokenBalance()).to.equal(ethers.parseEther("3000"));
    });

    it("Should not allow withdrawing more than available rewards", async function () {
      await staking.connect(owner).depositRewardTokens(ethers.parseEther("1000"));
      
      await expect(staking.connect(owner).withdrawRewardTokens(ethers.parseEther("2000")))
