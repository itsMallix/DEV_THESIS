...
describe("RewardToken", function () {
    ...
    it("Should fail if sender does not have enough balance", async function () {
        const transferAmount = ethers.parseEther("1000");
        await expect(rewardToken.connect(addr1).transfer(addr2.address, transferAmount)).to.be.revertedWith(
    ...
    it("Should allow approval and set allowance correctly", async function () {
        ...
        expect(await rewardToken.allowance(owner.address, addr1.address)).to.equal(approveAmount);
    });
    ...
    it("Should allow transferFrom when approved", async function () {
        ...
        await rewardToken.connect(addr1).transferFrom(owner.address, addr2.address, transferAmount);

        expect(await rewardToken.balanceOf(addr2.address)).to.equal(transferAmount);
        expect(await rewardToken.allowance(owner.address, addr1.address)).to.equal(0);
    });

    it("Should fail transferFrom if allowance is insufficient", async function () {
        const transferAmount = ethers.parseEther("3000");

        await expect(rewardToken.connect(addr1).transferFrom(owner.address, addr2.address, transferAmount))
