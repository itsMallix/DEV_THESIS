...
describe("TokenDrop", function () {
  ...
  beforeEach(async function () {
    ...
    // Approve TokenDrop biar bisa menghabiskan 100 token
    await rewardToken
      .connect(owner)
      .approve(await tokenDrop.getAddress(), parseUnits("100", 18));

    // depo 100 token ke sc tokendrop
    await tokenDrop.connect(owner).depositTokens(parseUnits("100", 18));
  });

  it("Should set the correct token address", async function () {
    expect(await tokenDrop.stakingToken()).to.equal(
  ...
  it("Should allow users to claim token once", async function () {
    ...
    await expect(tokenDrop.connect(addr1).claimToken()).to.be.revertedWith(
  ...
  it("Should allow only owner to deposit tokens", async function () {
    await expect(
      tokenDrop.connect(addr1).depositTokens(parseUnits("50", 18))
    ).to.be.revertedWith("Ownable: caller is not the owner");

    // Tambahkan approval sebelum deposit tambahan
    await rewardToken
      .connect(owner)
      .approve(await tokenDrop.getAddress(), parseUnits("50", 18));
    await tokenDrop.connect(owner).depositTokens(parseUnits("50", 18));

    expect(await rewardToken.balanceOf(tokenDrop.getAddress())).to.equal(
      parseUnits("150", 18)
    );
  });
  ...
  it("Should allow only owner to withdraw tokens", async function () {
    await expect(
      tokenDrop.connect(addr1).withdrawTokens(parseUnits("50", 18))
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await tokenDrop.connect(owner).withdrawTokens(parseUnits("50", 18));
    expect(await rewardToken.balanceOf(owner.address)).to.be.greaterThan(
      parseUnits("900", 18)
    );
  });

  it("Should update contract balance after deposits and withdrawals", async function () {
    expect(await tokenDrop.getDepositedSupply()).to.equal(
      parseUnits("100", 18)
    );
